create table yunocontent.content_types (
...
alter table yunocontent.content_types enable row level security;


create or replace function yunocontent.trfn_content_types_bu() returns trigger as $$
begin
...
create or replace trigger tr_content_types_bu before update
    on yunocontent.content_types
    for each row 
    execute function yunocontent.trfn_content_types_bu();

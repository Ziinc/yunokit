...
import { Loader2, Lock, Mail, Sparkles, Zap, Clock, Cloud, FileText, Users, Database, Wand2 } from "lucide-react";
...
const SignInPage: React.FC = () => {
  ...
  const testimonials = [
    ...
    {
      quote: "My design team used to joke that I was their personal HTML servant. With SupaContent, they're now independent content creators and I'm back to solving real engineering problems.",
    ...
    {
      quote: "My inbox used to be a graveyard of 'urgent' content change requests. Now our non-technical team handles everything themselves, and they say it's easier than using Word.",
    ...
    try {
      ...
      */
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Could not log in with demo account",
        variant: "destructive"
      });
    } finally {
  ...
  const handleOAuthSignIn = async (provider: 'google' | 'microsoft' | 'github') => {
    try {
      ...
      // OAuth redirect will happen automatically, no need to navigate
      // The auth callback will handle the redirect to the intended destination
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await signUpWithEmail(registerEmail, registerPassword);
  ...
  const mainFeatures = [
    {
      ...
      description: "Edit like a pro with our smart tools that make content creation magical and effortless.",
      features: ["Rich text editor", "Media library", "Version control", "Smart scheduling"]
    },
    {
      title: "Community Engagement",
      description: "Build thriving communities with tools designed for meaningful interactions.",
      features: ["Comment systems", "Interactive forums", "User profiles", "Moderation tools"]
    }
  ];
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-gradient-to-br from-indigo-100 via-purple-100 to-pink-100">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <Sparkles className="absolute top-[40%] left-[8%] text-yellow-400 h-5 w-5 animate-pulse" style={{ animationDelay: '1.5s' }} />
        <Sparkles className="absolute top-[30%] right-[20%] text-yellow-400 h-6 w-6 animate-pulse" style={{ animationDelay: '2.0s' }} />
        <Sparkles className="absolute top-[85%] right-[25%] text-yellow-400 h-5 w-5 animate-pulse" style={{ animationDelay: '1.0s' }} />
      </div>
        ...
        <Card className="w-full max-w-md shadow-xl z-10">
          <CardHeader className="text-center">
            <div className="mx-auto mb-4 w-48 h-12 flex items-center justify-center">
              <img src="/supacontent-logo.png" alt="SupaContent Logo" className="w-full h-full object-contain" />
            </div>
            <CardTitle className="text-2xl font-bold">{showRegister ? "Create an account" : "Welcome back"}</CardTitle>
            <CardDescription>
              {showRegister ? "Sign up to your account" : "Sign in to your account"}
            </CardDescription>
          </CardHeader>
                          ...
                          <DialogContent>
                            ...
                            <form onSubmit={handleResetPassword} className="space-y-4 py-4">
                              ...
                              <DialogFooter className="mt-4">
                                <Button type="button" variant="outline" onClick={() => setResetDialogOpen(false)} disabled={isResetSending}>
                                  Cancel
                                </Button>
                                <Button type="submit" disabled={isResetSending}>
                                  {isResetSending ? (
                                    <>
                                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                      Sending...
        ...
        <div className="hidden lg:flex flex-col max-w-xl w-full bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 rounded-2xl p-8 text-white shadow-xl">
          <div className="mb-5 flex items-center">
            ...
            <h2 className="text-3xl font-bold">
              Content Management <span className="relative italic font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-amber-400">
                Reimagined
          ...
          <div className="flex flex-col gap-2 mb-6">
            {features.map((feature, index) => (
              <div key={index} className="flex items-center hover:bg-white hover:bg-opacity-10 rounded-lg p-2 transition-all">
                <div className="mr-3 bg-white bg-opacity-20 rounded-full p-1.5 flex-shrink-0">
                  {feature.icon}
                </div>
                <div className="text-left">
                  <h3 className="font-medium text-sm">{feature.title}</h3>
                  <p className="text-xs text-white text-opacity-90 line-clamp-2">{feature.description}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-6 flex flex-col items-center justify-center">
            ...
            <p className="text-xs text-yellow-200 mt-1 italic opacity-80">*Hours measured in developer time, which somehow expands when estimating project deadlines yet shrinks during lunch breaks.</p>
          </div>
        </div>
      </div>
    </div>
